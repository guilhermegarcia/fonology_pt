...
- ⏱️ total para executar o código abaixo: **< 2s**
...
.pull-right[
- <mark>`biGramTbl()`</mark> $\rightarrow$ todos os bigramas
- Assim, podemos facilmente plotar padrões


### Visualizando bigramas

- Em conjunto com `biGramTbl()`:
- <mark>`plotBiGrams()`</mark> $\rightarrow$ gráficos com `ggplot2`
...
- O projeto se beneficiou do programa ENVOL (Université Laval). Mais informações: [gdgarcia.ca/fonology](https://gdgarcia.ca/fonology)
- Agradecimentos aos assistentes de pesquisa envolvidos no projeto (Linda Wong, Nicolas C. Bustos)